...
public class ScrollpHAT {
    ...
    public func clear() throws {
        buffer = [UInt8](count: 11, repeatedValue: UInt8(0x00))
        offset = 0
        try update()
    }
    ...
    public func setColumn(x: Int, value: UInt8) {
        if buffer.count <= x {
            buffer += [UInt8](count: x - buffer.count + 1, repeatedValue: UInt8(0x00))
        }
        buffer[x] = value
    }
    
    public func writeString(string: String, x: Int = 0) throws {
        var x = x
    ...
    public func scroll(delta: Int = 1) throws {
        offset += delta
        offset %= buffer.count
        try update()
    }
    
    public func scrollTo(position: Int = 0) throws {
        offset = position % buffer.count
        try update()
    }
